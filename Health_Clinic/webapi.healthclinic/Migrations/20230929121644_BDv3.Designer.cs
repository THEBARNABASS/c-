// <auto-generated />
using System;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Metadata;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;
using webapi.healthclinic.Contexts;

#nullable disable

namespace webapi.healthclinic.Migrations
{
    [DbContext(typeof(HealthClinicContext))]
    [Migration("20230929121644_BDv3")]
    partial class BDv3
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "7.0.11")
                .HasAnnotation("Relational:MaxIdentifierLength", 128);

            SqlServerModelBuilderExtensions.UseIdentityColumns(modelBuilder);

            modelBuilder.Entity("webapi.healthclinic.Domains.ClinicaDomain", b =>
                {
                    b.Property<Guid>("IdClinica")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("Cnpj")
                        .IsRequired()
                        .HasColumnType("VARCHAR(100)");

                    b.Property<DateTime>("HoraAbertura")
                        .HasColumnType("TIME");

                    b.Property<DateTime>("HoraFechamento")
                        .HasColumnType("TIME");

                    b.Property<Guid>("IdEndereco")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid?>("IdEnderecoNavigationIdEndereco")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("Nome")
                        .IsRequired()
                        .HasColumnType("VARCHAR(100)");

                    b.Property<string>("RazaoSocial")
                        .IsRequired()
                        .HasColumnType("VARCHAR(100)");

                    b.HasKey("IdClinica");

                    b.HasIndex("IdEnderecoNavigationIdEndereco");

                    b.ToTable("Clinicas");
                });

            modelBuilder.Entity("webapi.healthclinic.Domains.ConsultumDomain", b =>
                {
                    b.Property<Guid>("IdConsulta")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime>("DataAgendamento")
                        .HasColumnType("TIME");

                    b.Property<string>("Descricao")
                        .IsRequired()
                        .HasColumnType("VARCHAR(100)");

                    b.Property<Guid>("IdMedico")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid?>("IdMedicoNavigationIdMedico")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("IdPaciente")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid?>("IdPacienteNavigationIdPaciente")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("IdStatusConsulta")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid?>("IdStatusConsultaNavigationIdStatusConsulta")
                        .HasColumnType("uniqueidentifier");

                    b.HasKey("IdConsulta");

                    b.HasIndex("IdMedicoNavigationIdMedico");

                    b.HasIndex("IdPacienteNavigationIdPaciente");

                    b.HasIndex("IdStatusConsultaNavigationIdStatusConsulta");

                    b.ToTable("Consulta");
                });

            modelBuilder.Entity("webapi.healthclinic.Domains.EnderecoDomain", b =>
                {
                    b.Property<Guid>("IdEndereco")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("Bairro")
                        .IsRequired()
                        .HasColumnType("VARCHAR(100)");

                    b.Property<string>("Cep")
                        .IsRequired()
                        .HasMaxLength(8)
                        .HasColumnType("VARCHAR(100)");

                    b.Property<string>("Cidade")
                        .IsRequired()
                        .HasColumnType("VARCHAR(100)");

                    b.Property<string>("Estado")
                        .IsRequired()
                        .HasColumnType("VARCHAR(100)");

                    b.Property<string>("Logradouro")
                        .IsRequired()
                        .HasColumnType("VARCHAR(100)");

                    b.Property<string>("Nº")
                        .IsRequired()
                        .HasColumnType("VARCHAR(100)");

                    b.HasKey("IdEndereco");

                    b.ToTable("Enderecos");
                });

            modelBuilder.Entity("webapi.healthclinic.Domains.MedEspecialidadeDomain", b =>
                {
                    b.Property<Guid>("IdEspecialidade")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("NomeEspecialidae")
                        .IsRequired()
                        .HasColumnType("VARCHAR(100)");

                    b.HasKey("IdEspecialidade");

                    b.ToTable("MedEspecialidades");
                });

            modelBuilder.Entity("webapi.healthclinic.Domains.MedicoDomain", b =>
                {
                    b.Property<Guid>("IdMedico")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("Crm")
                        .IsRequired()
                        .HasMaxLength(11)
                        .HasColumnType("CHAR(60)");

                    b.Property<Guid>("IdClinica")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("IdEndereco")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("IdEspecialidade")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("IdUsuario")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("Nome")
                        .IsRequired()
                        .HasColumnType("VARCHAR(100)");

                    b.HasKey("IdMedico");

                    b.HasIndex("IdClinica");

                    b.HasIndex("IdEndereco");

                    b.HasIndex("IdEspecialidade");

                    b.HasIndex("IdUsuario");

                    b.ToTable("Medicos");
                });

            modelBuilder.Entity("webapi.healthclinic.Domains.PacienteDomain", b =>
                {
                    b.Property<Guid>("IdPaciente")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("Cpf")
                        .IsRequired()
                        .HasMaxLength(11)
                        .HasColumnType("CHAR(11)");

                    b.Property<DateTime>("DataNascimento")
                        .HasColumnType("TIME");

                    b.Property<Guid>("IdUsuario")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("Nome")
                        .IsRequired()
                        .HasColumnType("VARCHAR(100)");

                    b.Property<string>("Rg")
                        .IsRequired()
                        .HasMaxLength(14)
                        .HasColumnType("CHAR(14)");

                    b.Property<string>("Telefone")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("IdPaciente");

                    b.HasIndex("IdUsuario");

                    b.ToTable("Pacientes");
                });

            modelBuilder.Entity("webapi.healthclinic.Domains.StatusconsultumDomain", b =>
                {
                    b.Property<Guid>("IdStatusConsulta")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("TituloConsulta")
                        .IsRequired()
                        .HasColumnType("VARCHAR(100)");

                    b.HasKey("IdStatusConsulta");

                    b.ToTable("Statusconsulta");
                });

            modelBuilder.Entity("webapi.healthclinic.Domains.TipoUsuarioDomain", b =>
                {
                    b.Property<Guid>("IdTipoDeUsuario")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("NomeTipoUsuario")
                        .IsRequired()
                        .HasColumnType("VARCHAR(100)");

                    b.HasKey("IdTipoDeUsuario");

                    b.ToTable("TipoUsuarios");
                });

            modelBuilder.Entity("webapi.healthclinic.Domains.UsuarioDomain", b =>
                {
                    b.Property<Guid>("IdUsuario")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("Email")
                        .IsRequired()
                        .HasColumnType("VARCHAR(100)");

                    b.Property<Guid>("IdTipoUsuario")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("Senha")
                        .IsRequired()
                        .HasMaxLength(60)
                        .HasColumnType("CHAR(60)");

                    b.HasKey("IdUsuario");

                    b.HasIndex("Email")
                        .IsUnique();

                    b.HasIndex("IdTipoUsuario");

                    b.ToTable("UsuarioDomain");
                });

            modelBuilder.Entity("webapi.healthclinic.Domains.ClinicaDomain", b =>
                {
                    b.HasOne("webapi.healthclinic.Domains.EnderecoDomain", "IdEnderecoNavigation")
                        .WithMany()
                        .HasForeignKey("IdEnderecoNavigationIdEndereco");

                    b.Navigation("IdEnderecoNavigation");
                });

            modelBuilder.Entity("webapi.healthclinic.Domains.ConsultumDomain", b =>
                {
                    b.HasOne("webapi.healthclinic.Domains.MedicoDomain", "IdMedicoNavigation")
                        .WithMany()
                        .HasForeignKey("IdMedicoNavigationIdMedico");

                    b.HasOne("webapi.healthclinic.Domains.PacienteDomain", "IdPacienteNavigation")
                        .WithMany("Consulta")
                        .HasForeignKey("IdPacienteNavigationIdPaciente");

                    b.HasOne("webapi.healthclinic.Domains.StatusconsultumDomain", "IdStatusConsultaNavigation")
                        .WithMany("Consulta")
                        .HasForeignKey("IdStatusConsultaNavigationIdStatusConsulta");

                    b.Navigation("IdMedicoNavigation");

                    b.Navigation("IdPacienteNavigation");

                    b.Navigation("IdStatusConsultaNavigation");
                });

            modelBuilder.Entity("webapi.healthclinic.Domains.MedicoDomain", b =>
                {
                    b.HasOne("webapi.healthclinic.Domains.ClinicaDomain", "Clinica_fk")
                        .WithMany("Medicos")
                        .HasForeignKey("IdClinica")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("webapi.healthclinic.Domains.EnderecoDomain", "Endereco_fk")
                        .WithMany()
                        .HasForeignKey("IdEndereco")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("webapi.healthclinic.Domains.MedEspecialidadeDomain", "Especialidade_fk")
                        .WithMany("Medicos")
                        .HasForeignKey("IdEspecialidade")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("webapi.healthclinic.Domains.UsuarioDomain", "Usuario_fk")
                        .WithMany("Medicos")
                        .HasForeignKey("IdUsuario")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Clinica_fk");

                    b.Navigation("Endereco_fk");

                    b.Navigation("Especialidade_fk");

                    b.Navigation("Usuario_fk");
                });

            modelBuilder.Entity("webapi.healthclinic.Domains.PacienteDomain", b =>
                {
                    b.HasOne("webapi.healthclinic.Domains.UsuarioDomain", "IdUsuarioNavigation")
                        .WithMany("Pacientes")
                        .HasForeignKey("IdUsuario")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("IdUsuarioNavigation");
                });

            modelBuilder.Entity("webapi.healthclinic.Domains.UsuarioDomain", b =>
                {
                    b.HasOne("webapi.healthclinic.Domains.TipoUsuarioDomain", "TipoUsuario_fk")
                        .WithMany("Usuario")
                        .HasForeignKey("IdTipoUsuario")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("TipoUsuario_fk");
                });

            modelBuilder.Entity("webapi.healthclinic.Domains.ClinicaDomain", b =>
                {
                    b.Navigation("Medicos");
                });

            modelBuilder.Entity("webapi.healthclinic.Domains.MedEspecialidadeDomain", b =>
                {
                    b.Navigation("Medicos");
                });

            modelBuilder.Entity("webapi.healthclinic.Domains.PacienteDomain", b =>
                {
                    b.Navigation("Consulta");
                });

            modelBuilder.Entity("webapi.healthclinic.Domains.StatusconsultumDomain", b =>
                {
                    b.Navigation("Consulta");
                });

            modelBuilder.Entity("webapi.healthclinic.Domains.TipoUsuarioDomain", b =>
                {
                    b.Navigation("Usuario");
                });

            modelBuilder.Entity("webapi.healthclinic.Domains.UsuarioDomain", b =>
                {
                    b.Navigation("Medicos");

                    b.Navigation("Pacientes");
                });
#pragma warning restore 612, 618
        }
    }
}
